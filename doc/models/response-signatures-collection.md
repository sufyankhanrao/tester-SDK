
# Response Signatures Collection

## Structure

`ResponseSignaturesCollection`

## Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `Type` | `String` | Required, Constant | Resource Type<br>**Default**: `"SignaturesCollection"` | String getType() | setType(String type) |
| `List` | [`List<List6>`](../../doc/models/list-6.md) | Required | Resource Members | List<List6> getList() | setList(List<List6> list) |

## Example (as JSON)

```json
{
  "type": "SignaturesCollection",
  "list": {
    "signature": "",
    "resource": "Transaction",
    "resource_id": null,
    "id": "11e95f8ec39de8fbdb0a4f1a",
    "created_ts": null,
    "modified_ts": null
  }
}
```

